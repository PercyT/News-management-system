...
from apps.news.models import NewsCategroy, News, Banner
...
@staff_member_required(login_url='index')  # 如果没有这个页面的权限，重定向到哪里去
def index(request):
...
class WriteNewsView(View):
    def get(self, request):
        catogories = NewsCategroy.objects.all()
        context = {
    ...
    def post(self, request):
        ...
        if form.is_valid():
            ...
            category = NewsCategroy.objects.get(pk=category_id)
            News.objects.create(title=title,desc=desc,thumbnail=thumbnail,context=context,
...
class NewsListView(View):
    def get(self,request):
        ...
        if start or end:
            ...
            newes = newes.filter(pub_time__range=(make_aware(start_date), make_aware(end_date)))
        ...
        paginator = Paginator(newes, 2)
        page_obj = paginator.page(page)
        context_data = self.get_pagination_data(paginator,page_obj)
        context = {
            'categories': NewsCategroy.objects.all(),
            'newes': page_obj.object_list,
            'page_obj':page_obj,
            'paginator':paginator,
            'start':start,
            'end':end,
...
def upload_file(request):
    ...
    with open(os.path.join(settings.MEDIA_ROOT,name), 'wb') as fp:
        for chunk in file.chunks():
            fp.write(chunk)
    url = request.build_absolute_uri(settings.MEDIA_URL+name) # 这个方法已经保存了127.0.0.1:8000 建立一个文件的url
    return restful.result(data={'url':url})

@staff_member_required(login_url='index')
@require_GET
def qntoken(request):
    access_key = 'VErYlrHGsIr4o0NnCC0-zKCGPfy_0YwH8KXambhe'
    secret_key = 'I7WCMywA8-YqhH7l-mx_heZgtQmgWV56ac6IU0dc'
...
def add_banner(request):
    ...
    if form.is_valid():
        ...
        return restful.result(data={"banner_id":banner.pk,"priority":banner.priority})
    else:
        return restful.params_error(message=form.get_errors)
@permission_required(perm='news.add_banner',login_url='/')
...
class EditNewsView(View):
    ...
    def post(self,request):
        ...
        if form.is_valid():
            ...
            News.objects.filter(pk=pk).update(title=title, desc=desc, thumbnail=thumbnail, context=context,
                                              category=category)
            return restful.ok()
        else:
            return restful.params_error(message=form.get_errors())
